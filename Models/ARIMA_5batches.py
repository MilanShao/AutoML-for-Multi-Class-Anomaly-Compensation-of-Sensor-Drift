...
arima_model = auto_arima(y_train, suppress_warnings=True)

# Fit ARIMA model with the selected parameters
arima_fit = ARIMA(y_train, order=arima_model.order).fit()
...
# Make predictions
predictions = arima_fit.predict(start=len(y_train), end=len(y_train) + len(y_test) - 1, typ='levels')
