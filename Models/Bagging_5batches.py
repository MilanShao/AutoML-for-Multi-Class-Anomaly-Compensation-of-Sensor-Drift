...
for i in range(1, 6):  # Load batches 1 to 5 for training
    ...
    y_train = pd.Series(y_train).astype(int) - 1  # Adjust class values
...
# Use Bagging for classification with Decision Tree as the base estimator
base_estimator = DecisionTreeClassifier(max_depth=10)
bagging_classifier = BaggingClassifier(base_estimator=base_estimator, n_estimators=100, max_samples=1.0, max_features=1.0)
...
for i in range(6, 11):  # Load batches 6 to 10 for testing
    ...
    y_test = pd.Series(y_test).astype(int) - 1  # Adjust class values
